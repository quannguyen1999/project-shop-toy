...
public class AccountController {
    ...
    @DeleteMapping("/id")
    public ResponseEntity<CustomRespone> deleteAccount(@RequestBody Account account){
    	return  customValidateAndStatus.checkCustomRespon(accountService.deleteAccount(account.getUserName()));
    }

    //Tìm kiếm account bằng id
    @ApiOperation(value = "tìm kiếm tài khoản bằng id")
    @GetMapping("/id")
    public ResponseEntity<CustomRespone> findAccountById( @RequestBody Account account){
        return customValidateAndStatus.checkCustomRespon(accountService.findAccountByUserName(account.getUserName()));
    }

    //thêm account
    @ApiOperation(value = "thêm tài khoản")
    @PostMapping
    public ResponseEntity<CustomRespone> createNewAccount(@RequestBody @Valid Account account,Errors errors){
    ...
    @PutMapping
    public ResponseEntity<CustomRespone> updateAccount(@Valid @RequestBody Account account,Errors errors){
    	CustomRespone customReponse=customValidateAndStatus.getListApiError(errors);
